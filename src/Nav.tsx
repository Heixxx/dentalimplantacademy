...
const Nav = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex items-center gap-2 md:gap-3">
            <span className="material-symbols-outlined text-primary text-3xl md:text-4xl">dentistry</span>
            <div>
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setIsOpen(!isOpen)} className="bg-white text-primary border border-slate-200 shadow-sm focus:outline-none p-2 rounded-lg hover:bg-slate-50 transition flex items-center justify-center" aria-label="Toggle menu">
              <span className="material-symbols-outlined text-2xl transition-transform duration-300">{isOpen ? 'close' : 'menu'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-slate-200 shadow-2xl flex flex-col items-center py-8 space-y-6 animate-[fadeIn_0.2s_ease-in-out]">
