...
const SHOWCASE_DATA: Room[] = [
  { id: 0, imageSrc: "/gabinet.jpeg" },
  { id: 1, imageSrc: "/gabinet_praca.jpeg" },
  { id: 2, imageSrc: "/biuro.jpeg" },
  { id: 3, imageSrc: "ludziee.jpeg" },
  { id: 4, imageSrc: "budynek.jpeg" },
];

const Showcase = () => {
  const { t } = useTranslation();
  const rooms = t('showcase.rooms', { returnObjects: true }) as { title: string; subtitle: string }[];
      ...
      <div className="relative w-full h-[350px] sm:h-[400px] md:h-[600px] flex items-center justify-center mt-4 md:mt-8">
        <div className="relative w-[90%] sm:w-[90%] md:w-[50%] lg:max-w-5xl h-full flex justify-center items-center">
          {SHOWCASE_DATA.map((room, index) => (
            ...
            >
              <img alt={rooms[index]?.title ?? ''} className="w-full h-full object-cover" src={room.imageSrc} />
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-90 transition-opacity duration-300 flex items-end p-4 md:p-8">
                 <div>
                    <p className="text-accent text-[10px] md:text-xs font-bold uppercase tracking-wider mb-1">{rooms[index]?.subtitle ?? ''}</p>
                    <h3 className="text-white text-base sm:text-lg md:text-2xl font-display font-bold">{rooms[index]?.title ?? ''}</h3>
                 </div>
              </div>
            </div>
          ))}
